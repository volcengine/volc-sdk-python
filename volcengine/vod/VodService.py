...
class VodService(VodServiceConfig):
    ...
    # GetVideoDetail.
    #
    # @param request VodGetVideoDetailRequest
    # @return VodGetVideoDetailResponse
    # @raise Exception
    def get_video_detail(self, request):
        try:
            if sys.version_info[0] == 3:
                jsonData = MessageToJson(request, False, True)
                params = json.loads(jsonData)
                ...
                for k, v in params.items():
                    if isinstance(v, (int, float, bool, str, unicode)) is True:
                        ...
                        params[k] = json.dumps(v)
            res = self.get("GetVideoDetail", params)
        except Exception as Argument:
            try:
                resp = Parse(Argument.__str__(), VodGetVideoDetailResponse(), True)
            except Exception:
                raise Argument
            else:
                raise Exception(resp.ResponseMetadata.Error.Code)
        else:
            return Parse(res, VodGetVideoDetailResponse(), True)
